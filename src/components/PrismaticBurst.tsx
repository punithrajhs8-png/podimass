...
const PrismaticBurst = ({
    ...
    useEffect(() => {
        ...
        const onPointer = (e: PointerEvent) => {
            ...
            const targetX = Math.min(Math.max(x, 0), 1);
            const targetY = Math.min(Math.max(y, 0), 1);
    ...
    useEffect(() => {
        ...
        // Safety checks for uniform updates
        ...
        if (gradTex && renderer && Array.isArray(colors) && colors.length > 0) {
        ...
        } else if (program.uniforms.uColorCount) {
            program.uniforms.uColorCount.value = 0;
        }
        // Handle mixBlendMode (canvas style update is in separate effect)
    }, [intensity, speed, animationType, colors, distort, offset, rayCount]);

    return <div style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }} ref={containerRef} />;
};
